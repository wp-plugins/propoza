...
if (!class_exists('Propoza_Request')) :
    ...
    class Propoza_Request
    {
        ...
        public function register_scripts_and_styles()
        {
            ...
            if (!is_admin()) {
                $this->load_file(Propoza::slug .
                    '-script', '../assets/js/front.js', true);
                wp_localize_script(Propoza::slug .
                    '-script',
                    Propoza::slug .
                    '_' .
                    'request', array('ajax_url' => admin_url('admin-ajax.php')));
                $this->load_file(Propoza::slug .
                    '-style', '../assets/css/front.css');
            }
        }
        ...
        private function prepare_quote()
        {
            ...
            $to_propoza_quote['include_default_store_tax'] =
        ...
        private function prepare_products()
        {
            ...
            foreach ($woocommerce->cart->get_cart() as $key => $product) {
                if ($product['data']->post->post_parent >
                    0
                ) {
        ...
        {
            $response = wp_remote_post(Propoza::get_form_quote_request_url(), array('method' => 'POST', 'body' => json_encode($this->get_prepared_logged_in_user()), 'headers' => array('Content-Type' => 'text/html', 'Authorization' =>
                'Basic ' .
                get_option('wc_settings_tab_propoza_api_key', null))));
            echo $response['body'];
            die;
        }
        ...
        {
            $quote = $this->prepare_quote_request();
            $quote['Quote']['Requester'] = array_merge($quote['Quote']['Requester'], $_POST['form-data']);
            $response = wp_remote_post($_POST['form-action'], array('method' => 'POST', 'body' => json_encode($quote), 'headers' => array('Content-Type' => 'application/json', 'Authorization' =>
                'Basic ' .
                get_option('wc_settings_tab_propoza_api_key', null))));
            echo $response['body'];
            die;
